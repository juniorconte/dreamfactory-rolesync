...
function mirror(name, rules, catalog, force) {
  if (validate.initialized()) {
    var roleExists = !!fs.existsSync(`./roles/${name}.yaml`);

    if (!force && roleExists) {
      console.log(`Error: Role ${name} already exists and has not been modified`);
    } else {
